...
const G = {}, lvlOf = c => Math.min(5, 1 + Math.floor(c / 4));
...
function mathMenu() { clearInterval(G.timer); $('#math-game').classList.add('hidden'); $('#math-menu').classList.remove('hidden'); }
function mathStart(id) {
  Object.assign(G, { m: MODES.find(m => m.id === id), c: 0, n: 0, streak: 0, best: 0 });
  $('#math-menu').classList.add('hidden'); $('#math-game').classList.remove('hidden');
  id === 'mill' ? millStart() : mathNext();
}
function mathNext() {
  const L = lvlOf(G.c); G.cur = G.m.g(L);
  $('#math-game').innerHTML = `<div class="flex flex-wrap justify-between gap-2 font-bold mb-3"><button class="underline" data-mact="menu">← Modos</button><span>${G.m.t}</span><span>Nivel ${L}/5</span><span>Aciertos ${G.c}/${G.n}</span><span>Racha ${G.streak}</span></div>
    ${G.cur.svg || ''}<p class="text-xl my-3">${G.cur.q}</p>
    <form id="mform" class="flex flex-wrap gap-2"><input id="mans" autocomplete="off" class="border-2 border-ink rounded-lg p-2 flex-1 min-w-[10rem]" aria-label="Respuesta" placeholder="${esc(G.cur.ph)}"><button class="btn btn-primary">Comprobar</button></form>
...
function mathCheck() {
  const c = G.cur, u = parseNums($('#mans').value, c.multi); if (!u.length || $('#mans').disabled) return;
  ...
  const before = lvlOf(G.c); G.n++; if (ok) { G.c++; G.streak++; G.best = Math.max(G.best, G.streak); } else G.streak = 0;
  const up = lvlOf(G.c) > before ? `<p class="font-bold bg-mark inline-block px-2 rounded">¡Subiste al nivel ${lvlOf(G.c)}!</p>` : '';
  $('#mfb').innerHTML = `<p class="font-bold text-lg ${ok ? 'text-green-700' : 'text-red-700'}">${ok ? '¡Correcto!' : 'Todavía no.'}</p>${ok ? '' : `<p>Respuesta: <strong>${ex.map(fmt).join('; ')}${c.unit || ''}</strong></p>`}<p class="text-sm">${c.exp}</p>${up}<div><button class="btn btn-primary mt-3" data-mact="next">Siguiente</button></div>`;
  $('#mans').disabled = true; $('#mform button').disabled = true; $('[data-mact=next]').focus();
}
function mathEnd() {
  const pct = G.n ? Math.round(G.c / G.n * 100) : 0;
  $('#math-game').innerHTML = `<h3 class="font-display text-2xl font-extrabold">Resumen: ${G.m.t}</h3><p class="my-3">Aciertos: <strong>${G.c}</strong> de ${G.n} (${pct}%) · Nivel alcanzado: <strong>${lvlOf(G.c)}</strong> · Mejor racha: <strong>${G.best}</strong></p>
    <button class="btn btn-primary" data-mact="again">Jugar de nuevo</button> <button class="btn" data-mact="menu">Cambiar de modo</button>`;
}
...
function millRender() {
  ...
  if (M.st === 'end') { el.innerHTML = `<h3 class="font-display text-2xl font-extrabold">${M.msg}</h3><p class="my-3">Te llevas <strong>${M.won} puntos</strong>.</p><button class="btn btn-primary" data-mact="again">Jugar de nuevo</button> <button class="btn" data-mact="menu">Cambiar de modo</button>`; return; }
  const fb = M.st === 'fb';
...
function onGameClick(b) {
  ...
  if (a === 'menu') mathMenu();
  else if (a === 'again') mathStart(G.m.id);
  else if (a === 'next') mathNext();
