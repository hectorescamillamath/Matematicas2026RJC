...
function mathMenu() { clearInterval(G.timer); clearTimeout(G.nt); $('#math-game').classList.add('hidden'); $('#math-menu').classList.remove('hidden'); }
...
function mathStart(id) {
  clearInterval(G.timer); clearTimeout(G.nt);
  Object.assign(G, { m: MODES.find(m => m.id === id), c: 0, n: 0, streak: 0, best: 0, retry: false, dur: 0 });
...
function startArit(d) {
  clearInterval(G.timer); Object.assign(G, { dur: d, c: 0, n: 0, streak: 0, best: 0, retry: false });
  mathFrame(); mathNext();
